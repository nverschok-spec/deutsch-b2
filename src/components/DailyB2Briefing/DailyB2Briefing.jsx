  ...
  return (
    <Card className="animate-slide-up">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-white">Daily B2 Briefing</h2>
        <span className="text-violet-400" aria-hidden="true">✨</span>
      </div>

      {status === 'loading' && (
        <div className="mt-3">
          <Spinner label="Готовлю обзор…" />
        </div>
      )}
      ...
      {nv && (
        <>
          <p className="mt-3 text-xs uppercase tracking-wide text-slate-400">Nomen-Verb-Verbindung des Tages</p>
          ...
          >
            ...
            {showTranslation ? (
              ...
              <p className="text-xs text-violet-400 mt-1">Показать перевод</p>
            )}
          </button>
        </>
      )}
